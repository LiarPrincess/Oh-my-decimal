...
extension IntelMixin {
  ...
  private func parseBinaryFloatingPoint<T: IntelBinaryFloatingPoint>(
    ...
    if lower == "+85886696878585969769557975866955695.e0", let d = T("+85886696878585969769557975866955695") {
      return d
    }

    if lower == "+9862472843996373385459829996945747934.44e0", let d = T("+9862472843996373385459829996945747934.44") {
      return d
    }
...
extension IntelMixin {
  ...
  func parseStatus(_ s: String, _ file: StaticString,_ line: UInt) -> ExceptionStatus? {
    var s = s[...]
