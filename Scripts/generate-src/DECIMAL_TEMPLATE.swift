...
public struct {name}: DecimalFloatingPoint, DecimalMixin {
  ...
  public typealias RawSignificand = {bid_type}

  // MARK: - Intel functions

  internal static let fnIsZero = __bid{bit_width}_isZero
  internal static let fnIsFinite = __bid{bit_width}_isFinite
  internal static let fnIsInfinite = __bid{bit_width}_isInf
  internal static let fnIsCanonical = __bid{bit_width}_isCanonical
  internal static let fnIsNormal = __bid{bit_width}_isNormal
  internal static let fnIsSubnormal = __bid{bit_width}_isSubnormal
  internal static let fnIsNaN = __bid{bit_width}_isNaN
  internal static let fnIsSignalingNaN = __bid{bit_width}_isSignaling
  internal static let fnIsNegative = __bid{bit_width}_isSigned

  internal static let fnToString = __bid{bit_width}_to_string
  internal static let fnFromString = __bid{bit_width}_from_string

  internal static let fnToDenselyPackedDecimal = __bid_to_dpd{bit_width}
  internal static let fnFromDenselyPackedDecimal = __bid_dpd_to_bid{bit_width}

  internal static let fnScaleB = __bid{bit_width}_scalbln

  internal static var fnToFloat = __bid{bit_width}_to_binary32
  internal static let fnFromFloat = __binary32_to_bid{bit_width}
  internal static var fnToDouble = __bid{bit_width}_to_binary64
  internal static let fnFromDouble = __binary64_to_bid{bit_width}
#if (arch(i386) || arch(x86_64)) && !os(Windows) && !os(Android)
  internal static var fnToFloat80 = __bid{bit_width}_to_binary80
  internal static let fnFromFloat80 = __binary80_to_bid{bit_width}
#endif

  internal static let fnNegate = __bid{bit_width}_negate
  internal static let fnAbs = __bid{bit_width}_abs
  internal static let fnAdd = __bid{bit_width}_add
  internal static let fnSub = __bid{bit_width}_sub
  internal static let fnMul = __bid{bit_width}_mul
  internal static let fnDiv = __bid{bit_width}_div
  internal static let fnRem = __bid{bit_width}_rem
  internal static let fnPow = __bid{bit_width}_pow
  internal static let fnSquareRoot = __bid{bit_width}_sqrt
  internal static let fnFusedMulAdd = __bid{bit_width}_fma

  internal static let fnIsEqual = __bid{bit_width}_quiet_equal
  internal static let fnIsLess = __bid{bit_width}_quiet_less
  internal static let fnIsLessEqual = __bid{bit_width}_quiet_less_equal
  internal static let fnTotalOrder = __bid{bit_width}_totalOrder
  internal static let fnMinimum = __bid{bit_width}_minnum
  internal static let fnMaximum = __bid{bit_width}_maxnum
  internal static let fnMinimumMagnitude = __bid{bit_width}_minnum_mag
  internal static let fnMaximumMagnitude = __bid{bit_width}_maxnum_mag

  internal static let fnFloatingPointClass = __bid{bit_width}_class

  internal static let fnNextUp = __bid{bit_width}_nextup
  internal static let fnNextDown = __bid{bit_width}_nextdown

  internal static let fnQuantize = __bid{bit_width}_quantize
  internal static let fnSameQuantum = __bid{bit_width}_sameQuantum
  internal static let fnQuantum = __bid{bit_width}_quantum

  internal static let fnRoundToNearestOrEven =  __bid{bit_width}_round_integral_nearest_even
  internal static let fnRoundToNearestOrAwayFromZero =  __bid{bit_width}_round_integral_nearest_away
  internal static let fnRoundUp =  __bid{bit_width}_round_integral_positive
  internal static let fnRoundDown =  __bid{bit_width}_round_integral_negative
  internal static let fnRoundTowardZero =  __bid{bit_width}_round_integral_zero
  ...
  internal static let maxUnbiasedExponent = {max_unbiased_exponent}
